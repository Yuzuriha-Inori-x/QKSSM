...
class TSAV(nn.Module):
    ...
    def forward(self, audio_input, visual_input, visual_patch_input, qst_input, qst_len, qst_prompt_input):
        ...
        visual_feat_wei = visual_feat * modality_wei[:, 0].unsqueeze(-1).unsqueeze(-1)
        audio_feat_wei = audio_feat * modality_wei[:, 1].unsqueeze(-1).unsqueeze(-1)
        fused_va_feat = visual_feat_wei + audio_feat_wei  # [B, T, C]

        # * temporal perception
        top_k_index_sort = self.TemporalPerception(qst_prompt, fused_va_feat)  # [B, 1, Top_K]
